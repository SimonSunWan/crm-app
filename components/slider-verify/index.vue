<template>
  ...
  >
    <view class="slider-track" ref="trackRef">
      ...
      <view 
        class="slider-button" 
        :style="{ left: isVerified ? 'auto' : buttonLeft + 'px', right: isVerified ? buttonRight + 'px' : 'auto' }"
        @mousedown="onMouseDown"
        @touchstart="onTouchStart"
      >
        <text class="slider-icon" :class="{ 'iconfont': true, 'icon-duigou': isVerified, 'icon-youshuangjiantou': !isVerified }"></text>
      </view>
    </view>
  </view>
</template>
...
const emit = defineEmits(['verify', 'reset', 'handlerMove', 'update:value', 'passCallback'])
...
const { isVerified, isDragging, startX, buttonLeft, buttonRight, componentWidth } = toRefs(state)
