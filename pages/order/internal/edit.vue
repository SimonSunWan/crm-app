  ...
  <view class="edit-container">
    ...
    <view v-else class="edit-content">
              ...
              <view class="card-content">
                <view class="form-row">
                  <view class="form-item">
                    ...
                    <input v-model="item.partNumber" class="input-field" placeholder="备件料号" disabled />
                  </view>
                  <view class="form-item">
                    <text class="label">使用数量</text>
                    <input v-model="item.quantity" class="input-field" placeholder="使用数量" />
                  </view>
                </view>
                ...
                <view class="form-row">
                  <view class="form-item">
                    ...
                    <input v-model="item.oldPartCode" class="input-field" placeholder="旧件编码" />
                  </view>
                  <view class="form-item">
                    <text class="label">新件编码</text>
                    <input v-model="item.newPartCode" class="input-field" placeholder="新件编码" />
                  </view>
                </view>
              </view>
            </view>
              ...
              <view class="card-content">
                ...
                <view class="form-row">
                  <view class="form-item">
                    ...
                    <input v-model="item.amount" class="input-field" placeholder="费用金额(元)" />
                  </view>
                </view>
              </view>
            </view>
              ...
              <view class="card-content">
                ...
                <view class="form-row">
                  <view class="form-item">
                    ...
                    <input v-model="item.quantity" class="input-field" placeholder="维修数量" />
                  </view>
                  <view class="form-item">
                    <text class="label">系数</text>
                    <input v-model="item.coefficient" class="input-field" placeholder="系数" />
                  </view>
                </view>
              </view>
            </view>
      ...
      <view class="bottom-actions">
        ...
        <button v-if="currentStep > 0" class="action-btn prev" @click="prevStep">上一步</button>
        <button v-if="currentStep < 2" class="action-btn next" @click="nextStep">下一步</button>
        <button v-if="currentStep === 2" class="action-btn submit" @click="handleSubmit" :loading="submitLoading">保存并完成</button>
      </view>
    </view>
  </view>
</template>
...
const getOrderDetail = async () => {
  loading.value = true;
  try {
    const response = await InternalOrderService.getOrderById(orderId.value);
    if (response) {
      // 设置基本信息
      ...
      if (response.customer && response.vehicleModel && carModelData.value.length > 0) {
        ...
        const cascaderPath = findCascaderPath(carModelData.value, response.customer, response.vehicleModel);
        if (cascaderPath) {
      ...
      if (response.details && response.details.length > 0) {
        ...
        if (detail.partCategory && partCategoryData.value.length > 0) {
          ...
          if (detail.partCategory && detail.partLocation) {
            repairData.partSelection = [detail.partCategory, detail.partLocation];
          } else {
        ...
        labors.value = (detail.labors || []).map((labor) => {
          ...
          if (labor.repairSelection && repairItemsData.value.length > 0) {
            if (Array.isArray(labor.repairSelection)) {
              ...
              const repairCascaderPath = findRepairCascaderPath(repairItemsData.value, labor.repairSelection);
...
const initOptionsArrays = () => {
  const convertToUniDataPickerFormat = (data) => {
    return data.map(item => ({
      value: item.keyValue,
      text: item.dictValue,
      children: item.children ? convertToUniDataPickerFormat(item.children) : undefined
    }));
  };
  
  carModelData.value = convertToUniDataPickerFormat(dictionaryOptions.value.carModel || []);
  partCategoryData.value = convertToUniDataPickerFormat(dictionaryOptions.value.partCategory || []);
  
  projectTypeData.value = (dictionaryOptions.value.projectType || []).map(item => ({
    value: item.keyValue,
    text: item.dictValue
  }));
  projectStageData.value = (dictionaryOptions.value.projectPhase || []).map(item => ({
    value: item.keyValue,
    text: item.dictValue
  }));
  faultClassificationData.value = (dictionaryOptions.value.faultClassification || []).map(item => ({
    value: item.keyValue,
    text: item.dictValue
  }));
  faultLocationData.value = (dictionaryOptions.value.faultLocation || []).map(item => ({
    value: item.keyValue,
    text: item.dictValue
  }));
  spareLocationData.value = (dictionaryOptions.value.spareLocation || []).map(item => ({
    value: item.keyValue,
    text: item.dictValue
  }));
  partNumberData.value = (dictionaryOptions.value.partNumber || []).map(item => ({
    value: item.keyValue,
    text: item.dictValue
  }));
  ...
  repairItemsData.value = convertToUniDataPickerFormat(dictionaryOptions.value.repairItems || []);
};
