...
const handleAdd = () => {
  const params = ExternalOrderDataService.buildNavigationParams(dictionaryOptions.value);
  const queryString = Object.keys(params)
    .map(key => `${key}=${encodeURIComponent(params[key])}`)
    .join('&');
    
  uni.navigateTo({
    url: `/pages/order/external/add?${queryString}`
  });
};
...
const handleCardClick = (item) => {
  const params = ExternalOrderDataService.buildNavigationParams(dictionaryOptions.value, { id: item.id });
  const queryString = Object.keys(params)
    .map(key => `${key}=${encodeURIComponent(params[key])}`)
    .join('&');
    
  uni.navigateTo({
    url: `/pages/order/external/detail?${queryString}`
  });
};
...
const handleEdit = (item) => {
  ...
  uni.navigateTo({
    url: `/pages/order/external/edit?${queryString}`
  });
};
...
const loadDictionaryData = async () => {
  try {
    dictionaryOptions.value = await ExternalOrderDataService.loadDictionaryData();
  } catch (error) {
