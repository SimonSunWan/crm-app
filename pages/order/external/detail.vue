<template>
  <view class="detail-container">
    ...
    <view v-else-if="!orderData" class="empty-container">
      <uni-empty text="数据不存在"></uni-empty>
    </view>
    ...
    <view v-else class="detail-content">
      <view class="info-section">
        ...
        <view class="info-grid">
          <view class="info-item">
            ...
            <text class="value">{{ getCarModelLabel(orderData.customer) || '-' }}</text>
          </view>
          <view class="info-item">
            <text class="label">车型：</text>
            <text class="value">{{ getCarModelLabel(orderData.vehicleModel) || '-' }}</text>
          </view>
          <view class="info-item">
      ...
      <view class="info-section">
        ...
        <view class="info-grid">
          <view class="info-item">
            ...
            <text class="value">{{ getProjectTypeLabel(orderData.projectType) || '-' }}</text>
          </view>
          <view class="info-item">
            <text class="label">项目阶段：</text>
            <text class="value">{{ getProjectPhaseLabel(orderData.projectStage) || '-' }}</text>
          </view>
        </view>
      </view>
      ...
      <view class="info-section">
        ...
        <view class="info-grid">
          ...
          <view class="info-item">
            ...
            <text class="value">{{ orderData.mileage ? `${orderData.mileage}` : '-' }}</text>
          </view>
          <view class="info-item">
      ...
      <view class="info-section">
        ...
        <view class="info-grid">
          <view class="info-item">
            ...
            <view class="tag" :class="{ 'tag-success': orderData.underWarranty, 'tag-danger': !orderData.underWarranty }">
      ...
      <view v-if="getDetailValue('repairPerson')" class="info-section">
        ...
        <view class="info-grid">
          <view class="info-item">
            ...
            <text class="value">{{ getDetailValue('repairPerson') || '-' }}</text>
          </view>
          <view class="info-item">
          ...
          <view class="info-item">
            ...
            <view class="tag" :class="{ 'tag-success': getDetailValue('avicResponsibility'), 'tag-danger': !getDetailValue('avicResponsibility') }">
              {{ getDetailValue('avicResponsibility') ? '是' : '否' }}
            </view>
          </view>
          <view class="info-item">
            <text class="label">故障分类：</text>
            <text class="value">{{ getFaultClassificationLabel(getDetailValue('faultClassification')) || '-' }}</text>
          </view>
          <view class="info-item">
            <text class="label">故障位置：</text>
            <text class="value">{{ getFaultLocationLabel(getDetailValue('faultLocation')) || '-' }}</text>
          </view>
          <view class="info-item">
            <text class="label">零件类别/定位：</text>
            <text class="value">{{ getPartCategoryLabel(getDetailValue('partCategory'), getDetailValue('partLocation')) || '-' }}</text>
          </view>
          <view class="info-item full-width">
            <text class="label">维修描述：</text>
            <text class="value">{{ getDetailValue('repairDescription') || '-' }}</text>
          </view>
        </view>
      </view>
      ...
      <view v-if="getSpareParts().length > 0" class="info-section">
        ...
        <view class="info-grid">
          <view class="info-item">
            ...
            <text class="value">{{ getSpareLocationLabel(getDetailValue('sparePartLocation')) || '-' }}</text>
          </view>
        </view>
        
        <view v-for="(item, index) in getSpareParts()" :key="index" class="info-section">
          ...
          <view class="info-grid">
            ...
            <view class="info-item">
              ...
              <text class="value">{{ getPartNameLabel(item.name) || item.name || '-' }}</text>
            </view>
            <view class="info-item">
      ...
      <view v-if="getCosts().length > 0" class="info-section">
        ...
        <view v-for="(item, index) in getCosts()" :key="index" class="info-section">
          <view class="section-title">费用信息 {{ index + 1 }}</view>
          <view class="info-grid">
            <view class="info-item">
              <text class="label">费用类别：</text>
              <text class="value">{{ getFeeTypeLabel(item.category) || item.category || '-' }}</text>
            </view>
            <view class="info-item">
      ...
      <view v-if="getLabors().length > 0" class="info-section">
        ...
        <view v-for="(item, index) in getLabors()" :key="index" class="info-section">
          <view class="section-title">工时信息 {{ index + 1 }}</view>
          <view class="info-grid">
            <view class="info-item">
              <text class="label">故障位置/维修项目：</text>
              <text class="value">{{ getRepairSelectionText(item.repairSelection) || '-' }}</text>
            </view>
            <view class="info-item">
...
const hasDetailRecords = computed(() => {
  return getSpareParts().length > 0 || getCosts().length > 0 || getLabors().length > 0;
});
...
const getCarModelLabel = (keyValue) => DictionaryUtils.getDictionaryLabel(keyValue, dictionaryOptions.value.carModel, true);
const getProjectTypeLabel = (keyValue) => DictionaryUtils.getDictionaryLabel(keyValue, dictionaryOptions.value.projectType);
const getProjectPhaseLabel = (keyValue) => DictionaryUtils.getDictionaryLabel(keyValue, dictionaryOptions.value.projectPhase);
const getFaultClassificationLabel = (keyValue) => DictionaryUtils.getDictionaryLabel(keyValue, dictionaryOptions.value.faultClassification);
const getFaultLocationLabel = (keyValue) => DictionaryUtils.getDictionaryLabel(keyValue, dictionaryOptions.value.repairItems, true);
const getPartCategoryLabel = (categoryKey, locationKey) => {
  const category = DictionaryUtils.getDictionaryLabel(categoryKey, dictionaryOptions.value.partCategory, true);
  const location = DictionaryUtils.getDictionaryLabel(locationKey, dictionaryOptions.value.partCategory, true);
  if (category && location) {
    return `${category}/${location}`;
  }
  return category || location || null;
};
const getSpareLocationLabel = (keyValue) => DictionaryUtils.getDictionaryLabel(keyValue, dictionaryOptions.value.spareLocation);
const getPartNameLabel = (keyValue) => DictionaryUtils.getDictionaryLabel(keyValue, dictionaryOptions.value.partNumber);
const getFeeTypeLabel = (keyValue) => DictionaryUtils.getDictionaryLabel(keyValue, dictionaryOptions.value.feeType);
const getRepairItemLabel = (keyValue) => DictionaryUtils.getDictionaryLabel(keyValue, dictionaryOptions.value.repairItems, true);
...
const getRepairSelectionText = (repairSelection) => {
  ...
  if (Array.isArray(repairSelection)) {
    // 如果是数组，提取文本
    const texts = repairSelection.map(item => {
...
onLoad((options) => {
  ...
  dictionaryOptions.value = ExternalOrderDataService.parseNavigationParams(options);
